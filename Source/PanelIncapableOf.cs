...
public class PanelIncapableOf : PanelBase {
    protected List<string> cachedMissingWorkTypes;
    protected string missingWorkTypeString;
    protected Rect RectText;
    protected Vector2 SizeAlert = new(20, 20);

    public override string PanelHeader => "IncapableOf".Translate();

    public override void Resize(Rect rect) {
        base.Resize(rect);
        var panelPadding = new Vector2(11, 16);
        var textWidth = PanelRect.width - (panelPadding.x * 2);
        var textHeight = PanelRect.height - BodyRect.y - panelPadding.y;
    ...
    protected override void DrawPanelContent(State state) {
        ...
        var incapable = state.CurrentPawn.IncapableOf;
        if (incapable == null) {
