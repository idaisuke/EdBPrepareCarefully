...
public delegate void ModifyPawnCompPostLoad(Pawn pawn, Dictionary<string, ThingComp> thingCompLookup,
    HashSet<string> loadedComps);

public delegate void ModifyCustomPawnCompPostLoad(CustomPawn pawn, Dictionary<string, ThingComp> thingCompLookup,
    HashSet<string> loadedComps);

public class PawnCompPostLoadModifiers {
    public static List<ModifyPawnCompPostLoad> pawnFunctions = new();
    public static List<ModifyCustomPawnCompPostLoad> customPawnFunctions = new();
...
public static class DefaultPawnCompRules {
    ...
    public static void RemoveDefaultVanillaHairExtendedBeard(Pawn pawn, Dictionary<string, ThingComp> compLookup,
        ...
        if (!savedComps.Contains("VanillaHairExpanded.CompBeard")) {
            if (compLookup.TryGetValue("VanillaHairExpanded.CompBeard", out var c)) {
                var beardDef = ReflectionUtil.GetFieldValue<HairDef>(c, "beardDef");
                if (beardDef != null && !Equals(beardDef, "VHE_BeardCleanShaven")) {
                    var defaultBeardDef = DefDatabase<HairDef>.GetNamedSilentFail("VHE_BeardCleanShaven");
                    if (defaultBeardDef != null) {
                        ReflectionUtil.SetPublicField(c, "beardDef", defaultBeardDef);
                    }
                    else {
                        Logger.Warning(
                            "Vanilla Hairs Extended added a default beard because none was saved with the pawn preset.  We tried to remove it but failed.");
                    }
                }
            }
        }
    ...
    public static void RemoveDefaultFacialStuff(Pawn pawn, Dictionary<string, ThingComp> compLookup,
        HashSet<string> savedComps) {
        ...
        if (!savedComps.Contains("FacialStuff.CompFace")) {
            if (compLookup.TryGetValue("FacialStuff.CompFace", out var c)) {
                ReflectionUtil.InvokeActionMethod(c, "InitializeCompFace");
                var pawnFace = ReflectionUtil.GetPropertyValue<object>(c, "PawnFace");
                if (pawnFace == null) {
                    //Logger.Debug("Couldn't get the PawnFace value from the comp with class " + c.GetType().FullName);
                    return;
                }

                var beardDefType = ReflectionUtil.TypeByName("FacialStuff.Defs.BeardDef");
                if (beardDefType == null) {
                    //Logger.Debug("Didn't find the beard definition type");
                }
                else {
                    var defaultBeardDef = GenDefDatabase.GetDef(beardDefType, "Beard_Shaved", false);
                    if (defaultBeardDef == null) {
                ...
                var stacheDefType = ReflectionUtil.TypeByName("FacialStuff.Defs.MoustacheDef");
                if (stacheDefType == null) {
                    //Logger.Debug("Didn't find the moustache definition type");
                }
                else {
                    var defaultStacheDef = GenDefDatabase.GetDef(stacheDefType, "Shaved", false);
...
public class PawnCompInclusionRules : PawnCompRules {
    public List<string> compPrefixesToInclude = new();
    public HashSet<string> excludedFields = new();
    public HashSet<string> includedComps = new();

    public IEnumerable<string> IncludedComps => includedComps;

    public IEnumerable<string> ExcludedFields => excludedFields;
...
public class PawnCompExclusionRules : PawnCompRules {
    public List<string> compPrefixesToExclude = new();
    public HashSet<string> excludedComps = new();
    public HashSet<string> excludedFields = new();

    public IEnumerable<string> ExcludedComps => excludedComps;
