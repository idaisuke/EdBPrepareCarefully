...
namespace EdB.PrepareCarefully {
    public class DialogManageImplants : Window {
        ...
        protected void InitializeRecipes() {
            ...
            this.recipes.Clear();
            List<ImplantRecipe> result = new List<ImplantRecipe>();
            foreach (var recipe in health.ImplantRecipes) {
                ImplantRecipe implant = new ImplantRecipe();
                ...
                foreach (var part in health.FindBodyPartsForImplantRecipe(recipe)) {
                    ImplantBodyPart implantPart = new ImplantBodyPart();
                    implantPart.UniquePart = part;
                    Implant foundImplant = implantList.FirstOrDefault((Implant i) => {
                        return i.recipe == recipe && i.BodyPartRecord == part.Record;
                    });
        ...
        protected void ResetCachedBlockedSelectionAlert() {
            ...
            foreach (var item in blockedSelections) {
                listItems += "\n" +
                             "EdB.PC.Dialog.Implant.Alert.Item".Translate(item.recipe.LabelCap,
                                 item.BodyPartRecord.def.label);
            }
        ...
        protected void Resize() {
            ...
            table.AddColumn(new WidgetTable<ImplantRecipe>.Column() {
                ...
                DrawAction = (ImplantRecipe recipe, Rect rect, WidgetTable<ImplantRecipe>.Metadata metadata) => {
                    ...
                    if (!recipe.Disabled) {
                        Style.SetGUIColorForButton(labelRect, recipe.Selected, Style.ColorText,
                        ...
                        GUI.DrawTexture(checkboxRect,
                            recipe.Selected ? Textures.TextureCheckboxPartiallySelected : Textures.TextureCheckbox);
                        ...
                        if (recipe.BlockingImplant != null) {
                            TooltipHandler.TipRegion(labelRect,
                                "EdB.PC.Dialog.Implant.Conflict".Translate(recipe.BlockingImplant.recipe.LabelCap,
                                    recipe.BlockingImplant.BodyPartRecord.Label));
                        }
                    }
                    ...
                    if (recipe.Selected && recipe.RequiresPartSelection) {
                        ...
                        foreach (var part in recipe.Parts) {
                            ...
                            dottedLineRect = new Rect(labelRect.x, labelRect.y + 21, DottedLineSize.x,
                                DottedLineSize.y);
                            checkboxRect = new Rect(labelRect.x + labelRect.width - 22 - 6, labelRect.MiddleY() - 12,
                                22, 22);
                            clickRect = new Rect(labelRect.x, labelRect.y, labelRect.width - checkboxRect.width,
                                labelRect.height);
                            GUI.color = DottedLineColor;
                            GUI.DrawTexture(dottedLineRect, Textures.TextureDottedLine);
                            ...
                            if (!part.Disabled) {
                                Style.SetGUIColorForButton(labelRect, part.Selected, Style.ColorText,
                                    Style.ColorButtonHighlight, Style.ColorButtonHighlight);
                                ...
                                GUI.color = Color.white;
                                if (Widgets.ButtonImage(checkboxRect,
                                        part.Selected ? Textures.TextureCheckboxSelected : Textures.TextureCheckbox)) {
                                    ClickPartAction(recipe, part);
                                }
                            }
                            else {
                                ...
                                GUI.DrawTexture(checkboxRect,
                                    part.Selected
                                        ? Textures.TextureCheckboxPartiallySelected
                                        : Textures.TextureCheckbox);
                                if (Widgets.ButtonInvisible(checkboxRect)) {
                                    ClickPartAction(recipe, part);
                                }
                                ...
                                if (part.BlockingImplant != null) {
                                    TooltipHandler.TipRegion(labelRect,
                                        "EdB.PC.Dialog.Implant.Conflict".Translate(part.BlockingImplant.recipe.LabelCap,
                                            part.BlockingImplant.BodyPartRecord.Label));
                                }
                            }
