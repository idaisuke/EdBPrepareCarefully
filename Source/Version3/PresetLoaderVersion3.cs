...
public class PresetLoaderVersion3 {
    ...
    public bool Load(PrepareCarefully loadout, string presetName) {
        var pawns = new List<SaveRecordPawnV3>();
        var hiddenPawns = new List<SaveRecordPawnV3>();
        var savedRelationships = new List<SaveRecordRelationshipV3>();
        var parentChildGroups = new List<SaveRecordParentChildGroupV3>();
        Failed = false;
        ...
        try {
            ...
            Scribe_Values.Look(ref usePoints, "usePoints", true);
            Scribe_Values.Look(ref startingPoints, "startingPoints");
            Scribe_Values.Look<string>(ref ModString, "mods", "");

            try {
                Scribe_Collections.Look(ref pawns, "colonists", LookMode.Deep, null);
            }
            catch (Exception e) {
            ...
            try {
                Scribe_Collections.Look(ref hiddenPawns, "hiddenPawns", LookMode.Deep, null);
            }
            catch (Exception e) {
                Messages.Message("EdB.PC.Dialog.Preset.Error.Failed".Translate(), MessageTypeDefOf.ThreatBig);
                Logger.Warning("Error while loading preset", e);
            ...
            try {
                Scribe_Collections.Look(ref savedRelationships, "relationships", LookMode.Deep, null);
            }
            catch (Exception e) {
                Messages.Message("EdB.PC.Dialog.Preset.Error.Failed".Translate(), MessageTypeDefOf.ThreatBig);
                Logger.Warning("Error while loading preset", e);
            ...
            var tempEquipment = new List<SaveRecordEquipmentV3>();
            Scribe_Collections.Look(ref tempEquipment, "equipment", LookMode.Deep, null);
            loadout.Equipment.Clear();
            if (tempEquipment != null) {
                var equipment = new List<EquipmentSelection>(tempEquipment.Count);
        ...
        if (savedRelationships != null) {
            try {
                foreach (var r in savedRelationships) {
    ...
    public CustomPawn LoadPawn(SaveRecordPawnV3 record) {
        ...
        */
        ...
        for (var i = 0; i < record.skillNames.Count; i++) {
            var name = record.skillNames[i];
            var def = FindSkillDef(pawn.Pawn, name);
            if (def == null) {
        ...
        for (var i = 0; i < record.apparelLayers.Count; i++) {
            var layerIndex = record.apparelLayers[i];
            var layer = PrepareCarefully.Instance.Providers.PawnLayers.FindLayerFromDeprecatedIndex(layerIndex);
            if (layer == null) {
            ...
            var def = DefDatabase<ThingDef>.GetNamedSilentFail(record.apparel[i]);
            if (def == null) {
            ...
            if (!string.IsNullOrEmpty(record.apparelStuff[i])) {
                ...
                if (stuffDef == null) {
                    Logger.Warning("Could not load stuff definition \"" + record.apparelStuff[i] + "\" for apparel \"" +
                                   record.apparel[i] + "\"");
                    Failed = true;
                    continue;
                }
