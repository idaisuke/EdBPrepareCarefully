...
public static class UtilityCopy {
    ...
    // with no arguments.
    public static T CopyExposable<T>(T type, Dictionary<string, IExposable> crossReferences = null)
        where T : IExposable {
        var xml = "<doc>" + Scribe.saver.DebugOutputFor(type) + "</doc>";
        return CopyExposable(type, null, crossReferences);
    }
    ...
    public static T CopyExposable<T>(T type, object[] constructorArgs,
        Dictionary<string, IExposable> crossReferences = null) where T : IExposable {
        var xml = "<doc>" + Scribe.saver.DebugOutputFor(type) + "</doc>";
        //Logger.Debug(xml);
        ...
        var saveables = (HashSet<IExposable>)typeof(PostLoadIniter)
            .GetField("saveablesToPostLoad", BindingFlags.NonPublic | BindingFlags.Instance)
            .GetValue(Scribe.loader.initer);
        saveables.Clear();
        return result;
    }
    ...
    // with no arguments.
    public static void CopyExposableViaReflection(string fieldName, object source, object target,
        object[] constructorArgs) {
        var sourceField = source.GetType().GetField(fieldName);
        var targetField = target.GetType().GetField(fieldName);
        if (sourceField != null && targetField != null) {
            var value = sourceField.GetValue(source);
            if (typeof(IExposable).IsAssignableFrom(value.GetType())) {
                var e = (IExposable)value;
                var copy = CopyExposable(e, constructorArgs);
    ...
    private static void InitLoadFromString(String value) {
        ...
        try {
            ...
            Logger.Error(string.Concat(new object[] { "Exception while unmarshalling XML", "\n", ex }));
    ...
    public static void ResolveAllCrossReferences(Dictionary<string, IExposable> crossReferences = null) {
        ...
        using (List<IExposable>.Enumerator enumerator =
               Scribe.loader.crossRefs.crossReferencingExposables.GetEnumerator()) {
            while (enumerator.MoveNext()) {
                var loadReferenceable = enumerator.Current as ILoadReferenceable;
                if (loadReferenceable != null) {
                    var loadedObjectDirectory =
                        ReflectionUtil.GetFieldValue<LoadedObjectDirectory>(Scribe.loader.crossRefs,
                            "loadedObjectDirectory");
                    if (loadedObjectDirectory != null) {
                        loadedObjectDirectory.RegisterLoaded(loadReferenceable);
                    }
                    else {
                        Logger.Warning(
                            "Could not access CrossRefHandler.loadedObjectDirectory in our version of ResolveAllCrossReferences()");
                    }
                }
            }
        }

        foreach (var current in Scribe.loader.crossRefs.crossReferencingExposables) {
            try {
