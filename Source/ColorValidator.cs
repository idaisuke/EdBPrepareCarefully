...
public static class ColorValidator {
    public static Color ColorEmpty = new(-1f, -1f, -1f, -1f);
    ...
    public static bool Validate(ColorGenerator generator, Color color) {
        ...
        if (typeof(ColorGenerator_Single).Equals(generator.GetType())) {
            return Validate((ColorGenerator_Single)generator, color);
        }

        if (typeof(ColorGenerator_StandardApparel).Equals(generator.GetType())) {
            return Validate((ColorGenerator_StandardApparel)generator, color);
        }

        if (typeof(ColorGenerator_White).Equals(generator.GetType())) {
    ...
    public static bool Validate(ColorGenerator_Options generator, Color color) {
        foreach (var allowedColor in generator.options) {
            if (allowedColor.only != ColorEmpty) {
                if (color == allowedColor.only) {
                    return true;
                }
            }
            else if (color.r >= allowedColor.min.r && color.g >= allowedColor.min.g && color.b >= allowedColor.min.b &&
                     color.a >= allowedColor.min.a
                     && color.r <= allowedColor.max.r && color.g <= allowedColor.max.g &&
                     color.b <= allowedColor.max.b && color.a <= allowedColor.max.a) {
                return true;
            }
        }
