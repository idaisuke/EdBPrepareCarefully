...
public class ReflectionCache {
    ...
    public void Initialize() {
        ...
        GraphicDatabaseHeadRecords_BuildDatabaseIfNecessary =
            ReflectionUtil.RequiredMethod(typeof(GraphicDatabaseHeadRecords), "BuildDatabaseIfNecessary");
        GenFilePaths_FolderUnderSaveData =
            ReflectionUtil.RequiredMethod(typeof(GenFilePaths), "FolderUnderSaveData", new[] { typeof(string) });

        PawnBioAndNameGenerator_GetBackstoryCategoryFiltersFor = ReflectionUtil.RequiredMethod(
            typeof(PawnBioAndNameGenerator), "GetBackstoryCategoryFiltersFor",
            new[] { typeof(Pawn), typeof(FactionDef) });

        PawnBioAndNameGenerator_TryGetRandomUnusedSolidBioFor = ReflectionUtil.RequiredMethod(
            typeof(PawnBioAndNameGenerator), "TryGetRandomUnusedSolidBioFor",
            new[] {
                typeof(List<BackstoryCategoryFilter>), typeof(PawnKindDef), typeof(Gender), typeof(string),
                typeof(PawnBio).MakeByRefType()
            });
        PawnBioAndNameGenerator_IsBioUseable =
            ReflectionUtil.RequiredMethod(typeof(PawnBioAndNameGenerator), "IsBioUseable");
        ...
        ScenPart_ForcedHediff_PossibleHediffs =
            ReflectionUtil.RequiredMethod(typeof(ScenPart_ForcedHediff), "PossibleHediffs");

        Pawn_CachedDisabledWorkTypes = ReflectionUtil.RequiredField(typeof(Pawn), "cachedDisabledWorkTypes");
        Pawn_CachedDisabledWorkTypesPermanent =
            ReflectionUtil.RequiredField(typeof(Pawn), "cachedDisabledWorkTypesPermanent");
        PostLoadIniter_SaveablesToPostLoad =
            ReflectionUtil.RequiredField(typeof(PostLoadIniter), "saveablesToPostLoad");
        HediffComp_GetsPermanent_PainCategory =
            ReflectionUtil.RequiredField(typeof(HediffComp_GetsPermanent), "painCategory");
        PawnBioAndNameGenerator_FallbackCategoryGroup =
            ReflectionUtil.RequiredField(typeof(PawnBioAndNameGenerator), "FallbackCategoryGroup");
